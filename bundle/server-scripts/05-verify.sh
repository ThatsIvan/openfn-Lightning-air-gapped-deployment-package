...
MIGRATE_STATUS="$(docker compose ps migrate --format '{{.State}}' 2>/dev/null || true)"
if [ "${MIGRATE_STATUS}" = "exited" ]; then
  MIGRATE_EXIT="$(docker compose ps migrate --format '{{.ExitCode}}' 2>/dev/null || true)"
...
if docker compose logs worker --tail=50 2>/dev/null | grep -qi "connected\|Worker started OK\|worker ready"; then
  ok "Worker shows connected in logs"
else
  # Also check if the worker is at least running and claiming
  if docker compose logs worker --tail=50 2>/dev/null | grep -qi "claim\|starting"; then
...
echo "  Creating canary workflow via Lightning eval..."

# Create a project, webhook workflow, and trigger a run using pre-staged adaptors

CANARY_RESULT="$(docker compose exec -T web /app/bin/lightning eval '
  ...
  if [ -n "${WEBHOOK_PATH}" ]; then
    ...
    if [ "${TRIGGER_CODE}" = "200" ] || [ "${TRIGGER_CODE}" = "204" ] || [ "${TRIGGER_CODE}" = "202" ]; then
      ...
      while [ $WAITED -lt 60 ]; do
        sleep 5
        WAITED=$((WAITED + 5))
        # Check worker logs for canary output
        if docker compose logs worker --tail=30 2>/dev/null | grep -q "canary: adaptors work offline"; then
          CANARY_PASSED=true
          break
        fi
      done
      ...
      if [ "${CANARY_PASSED}" = true ]; then
        ...
        echo "       If you see 'module not found', the adaptor was not pre-staged."
      fi
    else
